...
function check_aruba_device( $host_id ) {
	$arubaurl = read_config_option("linkdiscovery_aruba_server");
...
//**** check if the device exist
	...
	$url = $arubaurl . '/network-device/name/' . $name;

	link_log("aruba device check exist: ". $url ."\n" );
	...
	if ( $result['http_code'] > "299" )
    {
		link_log("aruba device does not exist: ". $result['body'] ."\n" );
    }
	else {
		$itExist = true;
		link_log("aruba device existe: ". $result['body'] ."\n" );
	}
...
function add_aruba_device( $host_id ) {
	...
	if( check_aruba_device( $host_id ) ) {
		// device existe do not re-create it
		return;
	}
